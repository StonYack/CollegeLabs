...
char myName[NAME_MAX_LENGTH] = { '\0' };
char UDP_IP[NAME_MAX_LENGTH] = { '\0' };
char UDP_Port[NAME_MAX_LENGTH] = { '\0' };
...
static BaseType_t prvPostProcessTest(HTTPClient_t * pxClient)
{
    ...
    tempPointer = strstr(pxClient->pcRestData, "name=");
    if(tempPointer != NULL)
    {
        tempPointer += (int)strlen("name=");

        memset(myName, '\0', NAME_MAX_LENGTH);
		memset(UDP_IP, '\0', NAME_MAX_LENGTH);
		memset(UDP_Port, '\0', NAME_MAX_LENGTH);
        ...
        while(tempPointer[i] != '\0' && tempPointer[i] != '&')
        {
            ...
            {
                myName[i] = tempPointer[i];
            }
            i++;
        }
		//initializes tempPointer to be reused
		tempPointer = strstr(pxClient->pcRestData, "PortNumber=");
		tempPointer += strlen("PortNumber=");
		// resets i back to 0
		i = 0;
		// searches through the received values and adds it to the UDP_Port array
		while(tempPointer[i] != '\0' && tempPointer[i] != '&')
		{
			if(tempPointer[i] == '+')	
			{
			...
			{
				UDP_Port[i] = tempPointer[i];
			}
			i++;
		}
		//initializes tempPointer to be reused
		tempPointer = strstr(pxClient->pcRestData, "IP=");
		tempPointer += strlen("IP=");
		// resets i  back to 0
		i = 0;
		// searches through the received values and adds it to the UDP_IP array
		while(tempPointer[i] != '\0' && tempPointer[i] != '&')
		{
			if(tempPointer[i] == '+')
			{
			...
			{
				UDP_IP[i] = tempPointer[i];
			}
			i++;
		}	
		//Prints out the newly established values for debugging purposes
        FreeRTOS_printf(("%s\r\n", myName));
		FreeRTOS_printf(("%s\r\n", UDP_IP));
		FreeRTOS_printf(("%s\r\n", UDP_Port));
