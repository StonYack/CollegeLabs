...
QueueHandle_t xQueueLED;// = xQueueCreate(4, sizeof(char[50]));
QueueHandle_t xQueueRX;// = xQueueCreate(4, sizeof(char[50]));
QueueHandle_t xQueueTX;// = xQueueCreate(4, sizeof(char[50]));
...
int main (void)
{
	...
	xQueueLED = xQueueCreate(5, 1);
	xQueueRX = xQueueCreate(20, 1);
	xQueueTX = xQueueCreate(20, 50);
	...
	xTaskCreate(RXTask,				// Function Called by task
				...
				4,							// Task Priority
				NULL);						// Place to store Task Handle
	xTaskCreate(ButtonTask,				// Function Called by task
				"UART Control",			// Task Name
				configMINIMAL_STACK_SIZE,	// Task Stack Size
				NULL,						// Any Parameters Passed to Task
				4,							// Task Priority
				&xHandle);						// Place to store Task Handle
